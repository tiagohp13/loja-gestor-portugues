        ...
        <CardContent className="p-4">
          ...
          {(kpi.delta30dPct !== undefined || kpi.deltaMoMPct !== undefined) && (
            <div className="mt-2 flex items-center justify-center gap-3 text-xs font-medium text-gray-500">
              {/* Últimos 30 dias */}
              {typeof kpi.delta30dPct === "number" && (
                <span
                  className={`flex items-center gap-1 ${kpi.delta30dPct >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                  title="Variação nos últimos 30 dias"
                >
                  {kpi.delta30dPct >= 0 ? "▲" : "▼"} 30d {Math.abs(kpi.delta30dPct).toFixed(1)}%
                </span>
              )}
              ...
              {/* Mês vs Mês */}
              {typeof kpi.deltaMoMPct === "number" && (
                <span
                  className={`flex items-center gap-1 ${kpi.deltaMoMPct >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                  title="Variação do mês atual vs mês anterior"
                >
                  {kpi.deltaMoMPct >= 0 ? "▲" : "▼"} M/M {Math.abs(kpi.deltaMoMPct).toFixed(1)}%
                </span>
              )}
            </div>
          )}
          ...
          <div className="mt-2">
            ...
            <Progress value={progress} className={`h-2 ${belowTarget ? "bg-orange-100" : "bg-gray-100"}`} />
          </div>

          {belowTarget && (
            <Alert className="mt-2 py-2 bg-orange-50 border-orange-200">
              <AlertTriangle className="h-3.5 w-3.5 text-orange-500" />
              <AlertDescription className="text-xs text-orange-700">{getAlertMessage()}</AlertDescription>
            </Alert>
          )}
        </CardContent>
      </Card>
