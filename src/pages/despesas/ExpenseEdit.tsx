...
const ExpenseEdit = () => {
  ...
  const { suppliers, expenses, updateExpense } = useData();
  ...
  useEffect(() => {
    if (id) {
      ...
      if (foundExpense) {
        ...
        setFormData({
          supplierId: foundExpense.supplierId || '',
          supplierName: foundExpense.supplierName,
          date: new Date(foundExpense.date),
          notes: foundExpense.notes || '',
          discount: foundExpense.discount || 0,
          items: foundExpense.items.map(item => ({
  ...
  const handleSubmit = () => {
    if (!expense || !formData.supplierName || formData.items.length === 0) {
      return;
    }
    ...
    const updatedExpense: Expense = {
      ...
      items: formData.items.map((item, index) => ({
        id: expense.items[index]?.id || crypto.randomUUID(),
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <PageHeader 
        ...
        description="Alterar informações da despesa"
      />
