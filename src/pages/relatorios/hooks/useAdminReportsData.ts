...
export function useAdminReportsData({ startDate, endDate }: AdminReportsParams) {
  return useQuery({
    ...
    queryFn: async (): Promise<AdminReportsData> => {
      ...
      const totalSales = exits?.reduce((sum, exit) => {
      ...
      const previousSales = previousExits?.reduce((sum, exit) => {
        const itemsTotal = exit.stock_exit_items?.reduce((itemSum, item) => {
          const itemTotal = item.quantity * item.sale_price * (1 - (item.discount_percent || 0) / 100);
          return itemSum + itemTotal;
        }, 0) || 0;
      ...
      const clientPurchaseCounts = exits?.reduce((acc, exit) => {
        if (exit.client_id) {
      ...
      const highestMargin = topProducts.length > 0 
        ? { name: topProducts[0].name, margin: 45 } // Simplified - would need cost data
        : null;

      const lowestRotation = Object.entries(productSalesMap)
        .map(([name, data]) => ({ name, revenue: data.revenue }))
        .sort((a, b) => a.revenue - b.revenue)[0] || null;

      // KPI Indicators
      const totalOrders = exits?.length || 0;
      const totalItems = exits?.reduce((sum, exit) => {
